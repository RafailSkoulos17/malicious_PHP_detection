...
for root, subFolders, files in os.walk(rootdir_malicious):
    for _file in files:
        if _file.endswith(".php"):
            ...
            fo = FunctionOccurences(file_path)
            try:
                occurences_per_func = fo.get_occurences_in_percentage_per_function()
...
with open(os.path.join("outputs", "features", "function_percent_and_human_readable_features.json"), "w") as f:
